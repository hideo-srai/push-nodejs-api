...
exports.getEventsAnalyticsBySegment = function(req, res) {
  ...
  .then(function(devices) {
    ...
    var eventFilter = {
      ...
      eventType: customEvent.eventType || { $in: ['click', 'receive'] },
      eventValue: customEvent.eventValue || { $in: ['dpi', 'nova', 'supernova', 'meganova'] },
      created: {
...
exports.getEventsCountByWidgets = function(req, res) {
  var segmentId = req.query.segmentId;
  var startDate = req.query.startDate || '1980-01-01';
  var endDate = req.query.endDate || new Date();
  ...
  Promise.all([segmentPromise, userPromise])
  .then(function(result) {
    segment = result[0];
    userIds = _.map(result[1], '_id');
